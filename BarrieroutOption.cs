...
namespace HW1
{
    class BarrieroutOption:Option
    {
        public BarrieroutOption(double cst, double ck, double cr, double cT, int csteps, double csig, int ctrials, int cindex, bool cindex_anti, bool cindex_cv, bool cthreadingindex) : base(cst, ck, cr, cT, csteps, csig, ctrials, cindex, cindex_anti, cindex_cv, cthreadingindex)
        {
            s = cst;
            k = ck;
            r = cr;
        ...
        public override double optionprice_cv(out double se)
        {
            ...
            for (int i = 0; i < trials; i++)
            {
                ...
                if (index == 1)
                {
                    ...
                    {
                        int s1, s2;
                        if (stockmatrix[i, steps - 1] > stockmatrix[i, 0] || stockmatrix[i, steps - 1] < k)
                            s1 = 1;
                        else
                            s1 = 0;
                        if (stockmatrix[i + trials, steps - 1] > k || stockmatrix[i + trials, steps - 1] > stockmatrix[i, 0])
                            s2 = 1;
                        else
                            s2 = 0;
                        payoff[i, 0] = 0.5 * ((s1 + num * cv1) + (s2 + num * cv2));
                    }
                    else
                    {
                        if (stockmatrix[i, steps - 1] > stockmatrix[i, 0] || stockmatrix[i, steps - 1] < k)
