...
namespace HW1
{
    ...
    {
        public EuropeanOption(double cst, double ck, double cr, double cT, int csteps, double csig, int ctrials, int cindex,bool cindex_anti,bool cindex_cv,bool cthreadingindex) : base(cst, ck, cr, cT, csteps, csig, ctrials, cindex,cindex_anti,cindex_cv,cthreadingindex)
        {                                                                                                
            s = cst;
            k = ck;
            r = cr;
        ...
        public override double optionprice_cv(out double se)
        {
            ...
            for (int i = 0; i < trials; i++)                                                              
            {
                ...
                if (index == 1)                                                                               
                {
                    if (index_anti)                                                                                
                    {
                        payoff[i, 0] = 0.5 * ((Math.Max(stockmatrix[i, steps - 1] - k, 0) + num * cv1) + (Math.Max(stockmatrix[i + trials, steps - 1] - k, 0) + num * cv2));     
                    }
                    else                                                                                   
                    {
                        payoff[i, 0] = Math.Max(stockmatrix[i, steps - 1] - k, 0) + num * cv1;          
                    }
                }
                ...
                else                                                                                        
                {
                    if (index_anti)                                                                                   
                    {
                        payoff[i, 0] = 0.5 * ((Math.Max(k - stockmatrix[i, steps - 1], 0) + num * cv1) + (Math.Max(k - stockmatrix[i + trials, steps - 1], 0) + num * cv2));      
                    }
                    else                                                                                     
                    {
                        payoff[i, 0] = Math.Max(k - stockmatrix[i, steps - 1], 0) + num * cv1;              
                    }
                }
        ...
        public override double optionprice(out double se)                                                
        {
            ...
            if (index == 1)                                                                                   
            {
                ...
                {
                    payoff[i, 0] = Math.Max((stockmatrix[i, steps - 1] - k), 0);                                        
                }
            }
            ...
            if (index != 1)                                                                              
            {
                for (int i = 0; i < stockmatrix.GetLength(0); i++)
                {
                    payoff[i, 0] = Math.Max((k - stockmatrix[i, steps - 1]), 0);                          
                }
            }
