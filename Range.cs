...
namespace HW1
{
    class Range:Option
    {
        public Range(double cst, double ck, double cr, double cT, int csteps, double csig, int ctrials, int cindex, bool cindex_anti, bool cindex_cv, bool cthreadingindex) : base(cst, ck, cr, cT, csteps, csig, ctrials, cindex, cindex_anti, cindex_cv, cthreadingindex)
        {
            s = cst;
            k = ck;
            r = cr;
            ...
            for (int i = 0; i < trials; i++)
            {
                ...
                for (int j = 1; j < steps; j++)
                {
                    ...
                    for (int j = 0; j < stockmatrix.GetLength(1); j++)
                    {
                        ...
                        max2 =Math.Min( stockmatrix[i + trials, j],max2);
                        min1 = Math.Max(stockmatrix[i, j], min1);
                        min2 = Math.Min(stockmatrix[i + trials, j], min2);
                    }
                    payoff[i, 0] = 0.5 * ((Math.Max(max1 - min1, 0) + num * cv1) + (Math.Max(max2 - min2, 0) + num * cv2));
                    }
                    else
                    {
                    double max = 0;
                    ...
                    for (int j = 0; j < stockmatrix.GetLength(1); j++)
                    {
                        ...
                        min = Math.Max(stockmatrix[i, j], min);
                    }
                    payoff[i, 0] = Math.Max(max-min, 0) + num * cv1;
                    }
        ...
        public override double optionprice(out double se)
        {
            ...
            if (index_anti) { payoff = new double[trials * 2, 1]; }
                ...
                for (int i = 0; i < stockmatrix.GetLength(0); i++)
                {
                    ...
                    {
                        max = Math.Max(stockmatrix[i, j], max);
                        min = Math.Min(stockmatrix[i, j], min);
                    }
                    payoff[i, 0] = Math.Max((max-min), 0);
                }
